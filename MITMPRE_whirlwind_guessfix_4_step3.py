...
class Constraints_generator():
    ...
    def genConstraints_of_forwardRound(self, r):
        ...
        if r<self.TR:
            ...
            for j in range(64):
                constr = constr + MITMPreConstraints.P_guess_forward(input1_round[j],input2_round[j],input1_L_dual[j],input2_L_dual[j],Pgu[j])
            ...
            for j in range(8):
                if r%2==0:
                    ...
                    input1_col = col(input1_L_dual, j)
                    input2_col = col(input2_L_dual, j)
                    out1_col = col(input1_next, j)
                    out2_col = col(input2_next, j)                     
    ...
    def genConstraints_of_backwardRound(self, r):
        ...
        if r< self.TR:
            ...
            for j in range(64):
                constr = constr + MITMPreConstraints.P_guess_backward(input1_next[j],input2_next[j],input1_L_dual[j],input2_L_dual[j],Pgu[j])        
            ...
            for j in range(8):
                if r%2==0:
                    input1_col = row(input1_L_dual, j)
                    input2_col = row(input2_L_dual, j)
                    out1_col = row(input1_round, j)
                    out2_col = row(input2_round, j)
                else:
                    input1_col = col(input1_L_dual, j)
                    input2_col = col(input2_L_dual, j)
                    out1_col = col(input1_round, j)
                    out2_col = col(input2_round, j)                
    ...
    def genConstraints_minus1(self):
        ...
        if self.mat_r < self.ini_r:
            for j in range(8):                
                constr = constr + [BasicTools.plusTerm(out1[4+8*j:8+8*j])+ ' = 0']
                constr = constr + [BasicTools.plusTerm(out2[4+8*j:8+8*j])+ ' = 0']
                       
                constr = constr + MITMPreConstraints.equalConstraints(out1[8*j:8*j+4], input1[8*j:8*j+4])
                constr = constr + MITMPreConstraints.equalConstraints(out2[8*j:8*j+4], input2[8*j:8*j+4])                

        if self.mat_r > self.ini_r:
            for j in range(8):                
                constr = constr + [BasicTools.plusTerm(input1[4+8*j:8+8*j])+ ' = 0']
                constr = constr + [BasicTools.plusTerm(input2[4+8*j:8+8*j])+ ' = 0']
    ...
    def genConstraints_additional(self):
        ...
        constr = constr + [BasicTools.plusTerm(d1) + ' - ' + BasicTools.MinusTerm(CD2_b) + ' <= 24']
        constr = constr + [BasicTools.plusTerm(d2+Pguess_b) + ' - ' + BasicTools.MinusTerm(CD2_b) + ' <= 24']
        
       
        input1_mat = Vars_generator.genVars_input1_of_round(self.mat_r)
        input2_mat = Vars_generator.genVars_input2_of_round(self.mat_r)
        ...
        for j in range(8):
            if self.mat_r%2==0:
                ...
                constr = constr + [Gsum[j] + ' - ' + BasicTools.MinusTerm(col(matchvar1, j) + col(matchvar2, j)) + ' = 0']
    ...
    def genConstraints_total(self):
        ...
        # for j in range(8):                
            # constr = constr + [BasicTools.plusTerm(input1_round[4+8*j:8+8*j])+ ' = 4']
            # constr = constr + [BasicTools.plusTerm(input2_round[4+8*j:8+8*j])+ ' = 4']        
    ...
    def genModel(self, filename):
        ...
        fid = open('./Model/TR' + str(self.TR) + '_ini' + str(self.ini_r) + '_matr' + str(self.mat_r) + '_3.lp', 'w')
